 ...
 */
public class RequestAccessGoogle extends AsyncTask<Void, Void, String> {
    private Context context;
    private TaskService service;

    public RequestAccessGoogle(Context context, TaskService service) {
    ...
    protected void onPreExecute() {
        service.onExecute(SocialVariable.GOOGLE_REQUEST_ACCESS);
    }
    ...
    protected void onPostExecute(String string) {
        if (string != null) {
            service.onSuccess(SocialVariable.GOOGLE_REQUEST_ACCESS, string);
        } else {
            service.onError(SocialVariable.GOOGLE_REQUEST_ACCESS, context.getString(R.string.failed_recieve));
        }
    }
}
